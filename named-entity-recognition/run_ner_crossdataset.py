...
class DataTrainingArguments:
    ...
    labels_new: Optional[str] = field(
        default=None,
        metadata={"help": "Path to a file containing all new labels. If not specified, CoNLL-2003 labels are used."},
    )
    # labels: str = field(
    #     default=None,
    #     metadata={"help": "Path to a file containing all labels. If not specified, CoNLL-2003 labels are used."},
...
def main():
    ...
    labels_new = get_labels(data_args.labels_new)
    label_map_new: Dict[int, str] = {i: label for i, label in enumerate(labels_new)}
    num_labels_new = len(labels_new)
    ...
    model_old = AutoModelForTokenClassification.from_pretrained(
    ...
    def changeClassificationHead(model, new_num_labels):  # must pass in the full bert model

        newClassifier = nn.Linear(in_features=model.classifier.in_features, out_features=new_num_labels)
        new_model = copy.deepcopy(model)
        params = new_model.bert.parameters()
        for param in params:
            param.requires_grad = False
        new_model.classifier = newClassifier
        new_model.num_labels = new_num_labels

        return new_model

    model = changeClassificationHead(model_old, 7)
    ...
    def align_predictions(predictions: np.ndarray, label_ids: np.ndarray) -> Tuple[List[int], List[int]]:
        ...
        for i in range(batch_size):
            for j in range(seq_len):
                ...
                # if label_ids[i, j] != 7:
                    # print(f"label_ids[i][j]: {label_ids[i][j]}")
                    # print(f"label_map_new[label_ids[i][j]]: {label_map_new[label_ids[i][j]]}")
                    out_label_list[i].append(label_map_new[label_ids[i][j]])
                    # print(f"preds[i][j]: {preds[i][j]}")
                    # print(f"label_map_new[preds[i][j]]: {label_map_new[preds[i][j]]}")
                    preds_list[i].append(label_map_new[preds[i][j]])
